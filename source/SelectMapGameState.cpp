...
void SelectMapGameState::PerformActivate()
{
	...
	else
	{
		for (int i = 0; i < 2; ++i)
		{
			const char *szText = (i == 0) ? "Create New Field..." : "Load Tilt Field...";
	...
	if (pFiles)
	{
		...
		std::list<s3eLocation> corners;
		s3eLocation currLoc;
		
		g_pMapGame->GetLocation(currLoc);
		...
		while (S3E_RESULT_SUCCESS == s3eFileListNext(pFiles, szFile, 200))
		{
			if (strstr(szFile, ".map"))
			{
				...
				double distanceInMeters = LiveMaps::CalculateDistance(center, currLoc);
...
void SelectMapGameState::OnClickMapItem(CIwUIElement* Clicked)
{
	...
	else if ((CIwUIButton*)Clicked == g_pTiltButton)
	{
		GPSRectangle* pRect = g_pMapGame->GetBackground()->GetScaler()->GetCorners();

		g_loadedCorners.clear();
		g_loadedCorners.push_back(pRect->topLeft);
		g_loadedCorners.push_back(pRect->topRight);
		g_loadedCorners.push_back(pRect->bottomLeft);
		g_loadedCorners.push_back(pRect->bottomRight);

		g_pMapGame->SetScaledCorners(&g_loadedCorners, g_pMapGame->GetBackground()->GetZoom());
		g_pMapGame->SetLoadedMap("-tilt-", true);
		g_pMapGame->SetGameState(GPS_GameState_SelectGame, AnimDir_Left);
	}
	else
	{
		...
		else
		{
			...
			{
				if (g_loadedCorners.size() == 4)
				{
					g_pMapGame->SetScaledCorners(&g_loadedCorners, zoom);
					g_pMapGame->SetLoadedMap(fileName, false);
					g_pMapGame->SetGameState(GPS_GameState_SelectGame, AnimDir_Left);
...
bool SelectMapGameState::LoadMap(char* szMap, std::list<s3eLocation>* pCorners, float* pZoom)
{
	pCorners->clear();
	...
	if (pFile)
	{
		...
		bool result = Utils::LoadMap(gResult, pCorners, pZoom);
